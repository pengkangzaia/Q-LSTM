...
def get_msl_channel_ids():
    df = pd.read_csv('data/smap_msl/labeled_anomalies.csv')
    msls = df[df['spacecraft'] == 'MSL']
    # 55 channels in total
    channel_ids = [i.split('.')[0] for i in msls['chan_id'].values]
    return channel_ids


def train_msl(seq_length: int = 4):
    channel_ids = get_msl_channel_ids()
    for i in range(len(channel_ids)):
        ...
        for j in range(col_num):
            ...
            torch.save(lstm.state_dict(), 'trained_model/msl/saved_model_{}_{}'.format(channel_ids[i], j))
            ...
            plt.suptitle('MSL Training for channel {} {}th column'.format(channel_ids[i], j))
            plt.legend()
            plt.show()
...
def get_labels(channel_id: str, seq_length: int = 4):
    ...
    # 初始化数组，设置异常位置值为1
    labels = np.zeros(int(channel_info['num_values']))
...
def test_msl_for_all_channel(seq_length: int = 4):
    preds, accs = [], []
    channel_ids = get_msl_channel_ids()
    for channel_id in channel_ids:
        pred, acc = test_msl_for_channel(channel_id, seq_length)
        preds.append(pred)
        accs.append(acc)
...
def test_msl_for_channel(channel_id: str, seq_length: int = 4):
    abnormal_rates = []
    abnormals = []
    ...
    for idx in range(col_num):
        ...
        if torch.cuda.is_available():
            model.load_state_dict(torch.load('trained_model/msl/saved_model_{}_{}'.format(channel_id, idx)))
        else:
            model.load_state_dict(
                torch.load('trained_model/msl/saved_model_{}_{}'.format(channel_id, idx),
        ...
        plt.plot(dataY_plot, color='green', label='origin')
        plt.plot(data_predict_low, color='red', label='low quantile')
        plt.suptitle('MSL Test for channel {} {}th column'.format(channel_id, idx))
        plt.legend()
        plt.savefig('saved_fig/msl/msl_pred_{}_{}'.format(channel_id, idx))
        plt.show()
